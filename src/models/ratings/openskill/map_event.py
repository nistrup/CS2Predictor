"""team_map_openskill table model."""
...
class TeamMapOpenSkill(TeamMapSpecificEventMixin, Base):
    """Historical map-specific team OpenSkill events (one row per team per map)."""

    __tablename__ = "team_map_openskill"
    __table_args__ = (
        UniqueConstraint(
            "openskill_system_id",
            "team_id",
            "map_id",
            name="uq_team_map_openskill_system_team_map",
        ),
        CheckConstraint("actual_score IN (0.0, 1.0)", name="ck_team_map_openskill_actual_score"),
        CheckConstraint(
            "expected_score >= 0.0 AND expected_score <= 1.0",
            name="ck_team_map_openskill_expected_score",
        ),
        CheckConstraint("map_blend_weight >= 0.0 AND map_blend_weight <= 1.0", name="ck_team_map_openskill_blend_weight"),
        CheckConstraint("map_games_played_pre >= 0", name="ck_team_map_openskill_games_pre"),
        CheckConstraint("map_prior_games > 0.0", name="ck_team_map_openskill_map_prior_games"),
        CheckConstraint("pre_global_sigma > 0.0", name="ck_team_map_openskill_pre_global_sigma"),
        CheckConstraint("pre_map_sigma > 0.0", name="ck_team_map_openskill_pre_map_sigma"),
        CheckConstraint("pre_effective_sigma > 0.0", name="ck_team_map_openskill_pre_effective_sigma"),
        CheckConstraint("post_global_sigma > 0.0", name="ck_team_map_openskill_post_global_sigma"),
        CheckConstraint("post_map_sigma > 0.0", name="ck_team_map_openskill_post_map_sigma"),
        CheckConstraint("post_effective_sigma > 0.0", name="ck_team_map_openskill_post_effective_sigma"),
        Index("idx_team_map_openskill_system", "openskill_system_id"),
        Index(
            "idx_team_map_openskill_system_team_event",
            "openskill_system_id",
            "team_id",
            "event_time",
            "map_id",
        ),
        Index("idx_team_map_openskill_map_name", "openskill_system_id", "map_name", "post_effective_mu"),
        Index("idx_team_map_openskill_match", "match_id"),
        Index("idx_team_map_openskill_map", "map_id"),
    )
