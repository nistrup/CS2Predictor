"""player_match_openskill table model."""
...
class PlayerMatchOpenSkill(PlayerMatchEventMixin, Base):
    """Historical player OpenSkill events (one row per player per match)."""

    __tablename__ = "player_match_openskill"
    __table_args__ = (
        UniqueConstraint(
            "openskill_system_id",
            "player_id",
            "match_id",
            name="uq_player_match_openskill_system_player_match",
        ),
        CheckConstraint("actual_score IN (0.0, 1.0)", name="ck_player_match_openskill_actual_score"),
        CheckConstraint(
            "expected_score >= 0.0 AND expected_score <= 1.0",
            name="ck_player_match_openskill_expected_score",
        ),
        CheckConstraint("pre_sigma > 0.0", name="ck_player_match_openskill_pre_sigma"),
        CheckConstraint("post_sigma > 0.0", name="ck_player_match_openskill_post_sigma"),
        Index("idx_player_match_openskill_system", "openskill_system_id"),
        Index(
            "idx_player_match_openskill_system_player_event",
            "openskill_system_id",
            "player_id",
            "event_time",
            "match_id",
        ),
        Index("idx_player_match_openskill_match", "match_id"),
    )
