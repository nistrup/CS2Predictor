"""team_match_openskill table model."""
...
class TeamMatchOpenSkill(TeamMatchEventMixin, Base):
    """Historical team OpenSkill events (one row per team per match)."""

    __tablename__ = "team_match_openskill"
    __table_args__ = (
        UniqueConstraint(
            "openskill_system_id",
            "team_id",
            "match_id",
            name="uq_team_match_openskill_system_team_match",
        ),
        CheckConstraint("actual_score IN (0.0, 1.0)", name="ck_team_match_openskill_actual_score"),
        CheckConstraint(
            "expected_score >= 0.0 AND expected_score <= 1.0",
            name="ck_team_match_openskill_expected_score",
        ),
        CheckConstraint("pre_sigma > 0.0", name="ck_team_match_openskill_pre_sigma"),
        CheckConstraint("post_sigma > 0.0", name="ck_team_match_openskill_post_sigma"),
        Index("idx_team_match_openskill_system", "openskill_system_id"),
        Index(
            "idx_team_match_openskill_system_team_event",
            "openskill_system_id",
            "team_id",
            "event_time",
            "match_id",
        ),
        Index("idx_team_match_openskill_match", "match_id"),
    )
