"""player_match_glicko2 table model."""
...
class PlayerMatchGlicko2(PlayerMatchEventMixin, Base):
    """Historical player Glicko-2 events (one row per player per match)."""

    __tablename__ = "player_match_glicko2"
    __table_args__ = (
        UniqueConstraint(
            "glicko2_system_id",
            "player_id",
            "match_id",
            name="uq_player_match_glicko2_system_player_match",
        ),
        CheckConstraint("actual_score IN (0.0, 1.0)", name="ck_player_match_glicko2_actual_score"),
        CheckConstraint(
            "expected_score >= 0.0 AND expected_score <= 1.0",
            name="ck_player_match_glicko2_expected_score",
        ),
        CheckConstraint("pre_rd > 0.0", name="ck_player_match_glicko2_pre_rd"),
        CheckConstraint("post_rd > 0.0", name="ck_player_match_glicko2_post_rd"),
        CheckConstraint("pre_volatility > 0.0", name="ck_player_match_glicko2_pre_volatility"),
        CheckConstraint("post_volatility > 0.0", name="ck_player_match_glicko2_post_volatility"),
        Index("idx_player_match_glicko2_system", "glicko2_system_id"),
        Index(
            "idx_player_match_glicko2_system_player_event",
            "glicko2_system_id",
            "player_id",
            "event_time",
            "match_id",
        ),
        Index("idx_player_match_glicko2_match", "match_id"),
    )
