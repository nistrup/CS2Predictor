...
class PlayerGlicko2(PlayerMapEventMixin, Base):
    """Historical player Glicko-2 events (one row per player per map)."""

    __tablename__ = "player_glicko2"
    __table_args__ = (
        UniqueConstraint(
            "glicko2_system_id",
            "player_id",
            "map_id",
            name="uq_player_glicko2_system_player_map",
        ),
        CheckConstraint("actual_score IN (0.0, 1.0)", name="ck_player_glicko2_actual_score"),
        CheckConstraint(
            "expected_score >= 0.0 AND expected_score <= 1.0",
            name="ck_player_glicko2_expected_score",
        ),
        CheckConstraint("pre_rd > 0.0", name="ck_player_glicko2_pre_rd"),
        CheckConstraint("post_rd > 0.0", name="ck_player_glicko2_post_rd"),
        CheckConstraint("pre_volatility > 0.0", name="ck_player_glicko2_pre_volatility"),
        CheckConstraint("post_volatility > 0.0", name="ck_player_glicko2_post_volatility"),
        Index("idx_player_glicko2_system", "glicko2_system_id"),
        Index(
            "idx_player_glicko2_system_player_event",
            "glicko2_system_id",
            "player_id",
            "event_time",
            "map_id",
        ),
        Index("idx_player_glicko2_match", "match_id"),
        Index("idx_player_glicko2_map", "map_id"),
    )
