"""player_map_glicko2 table model."""
...
class PlayerMapGlicko2(PlayerMapSpecificEventMixin, Base):
    """Historical map-specific player Glicko-2 events (one row per player per map)."""

    __tablename__ = "player_map_glicko2"
    __table_args__ = (
        UniqueConstraint(
            "glicko2_system_id",
            "player_id",
            "map_id",
            name="uq_player_map_glicko2_system_player_map",
        ),
        CheckConstraint("actual_score IN (0.0, 1.0)", name="ck_player_map_glicko2_actual_score"),
        CheckConstraint(
            "expected_score >= 0.0 AND expected_score <= 1.0",
            name="ck_player_map_glicko2_expected_score",
        ),
        CheckConstraint("map_blend_weight >= 0.0 AND map_blend_weight <= 1.0", name="ck_player_map_glicko2_blend_weight"),
        CheckConstraint("map_games_played_pre >= 0", name="ck_player_map_glicko2_games_pre"),
        CheckConstraint("map_prior_games > 0.0", name="ck_player_map_glicko2_map_prior_games"),
        CheckConstraint("pre_global_rd > 0.0", name="ck_player_map_glicko2_pre_global_rd"),
        CheckConstraint("pre_map_rd > 0.0", name="ck_player_map_glicko2_pre_map_rd"),
        CheckConstraint("pre_effective_rd > 0.0", name="ck_player_map_glicko2_pre_effective_rd"),
        CheckConstraint("post_global_rd > 0.0", name="ck_player_map_glicko2_post_global_rd"),
        CheckConstraint("post_map_rd > 0.0", name="ck_player_map_glicko2_post_map_rd"),
        CheckConstraint("post_effective_rd > 0.0", name="ck_player_map_glicko2_post_effective_rd"),
        CheckConstraint("pre_global_volatility > 0.0", name="ck_player_map_glicko2_pre_global_vol"),
        CheckConstraint("pre_map_volatility > 0.0", name="ck_player_map_glicko2_pre_map_vol"),
        CheckConstraint("pre_effective_volatility > 0.0", name="ck_player_map_glicko2_pre_effective_vol"),
        CheckConstraint("post_global_volatility > 0.0", name="ck_player_map_glicko2_post_global_vol"),
        CheckConstraint("post_map_volatility > 0.0", name="ck_player_map_glicko2_post_map_vol"),
        CheckConstraint("post_effective_volatility > 0.0", name="ck_player_map_glicko2_post_effective_vol"),
        Index("idx_player_map_glicko2_system", "glicko2_system_id"),
        Index(
            "idx_player_map_glicko2_system_player_event",
            "glicko2_system_id",
            "player_id",
            "event_time",
            "map_id",
        ),
        Index("idx_player_map_glicko2_map_name", "glicko2_system_id", "map_name", "post_effective_rating"),
        Index("idx_player_map_glicko2_match", "match_id"),
        Index("idx_player_map_glicko2_map", "map_id"),
    )
