...
class PlayerMapElo(PlayerMapSpecificEventMixin, Base):
    """Historical map-specific player Elo events (one row per player per map)."""

    __tablename__ = "player_map_elo"
    __table_args__ = (
        UniqueConstraint("elo_system_id", "player_id", "map_id", name="uq_player_map_elo_system_player_map"),
        CheckConstraint("actual_score IN (0.0, 1.0)", name="ck_player_map_elo_actual_score"),
        CheckConstraint(
            "expected_score >= 0.0 AND expected_score <= 1.0",
            name="ck_player_map_elo_expected_score",
        ),
        CheckConstraint("map_blend_weight >= 0.0 AND map_blend_weight <= 1.0", name="ck_player_map_elo_blend_weight"),
        CheckConstraint("map_games_played_pre >= 0", name="ck_player_map_elo_games_pre"),
        CheckConstraint("map_prior_games > 0.0", name="ck_player_map_elo_map_prior_games"),
        Index("idx_player_map_elo_system", "elo_system_id"),
        Index(
            "idx_player_map_elo_system_player_event",
            "elo_system_id",
            "player_id",
            "event_time",
            "map_id",
        ),
        Index("idx_player_map_elo_map_name", "elo_system_id", "map_name", "post_effective_elo"),
        Index("idx_player_map_elo_match", "match_id"),
        Index("idx_player_map_elo_map", "map_id"),
    )
