"""Match-level player OpenSkill logic (Plackett-Luce model)."""
...
from domain.ratings.common import PlayerMatchResult
...
class PlayerMatchOpenSkillEvent:
    player_id: int
    team_id: int
...
class PlayerMatchOpenSkillCalculator(PlayerCalculatorMixin, TeamOpenSkillCalculator):
    """Stateful match-by-match player OpenSkill calculator."""

    def __init__(self, params: OpenSkillParameters) -> None:
        super().__init__(params=params)

    def process_match(self, match_result: PlayerMatchResult) -> list[PlayerMatchOpenSkillEvent]:
        self._validate_player_match(match_result)

        team1_pre = [self._get_or_create_rating(player.player_id) for player in match_result.team1_players]
        team2_pre = [self._get_or_create_rating(player.player_id) for player in match_result.team2_players]
        ...
        events: list[PlayerMatchOpenSkillEvent] = []

        for index, participant in enumerate(match_result.team1_players):
        ...
        for index, participant in enumerate(match_result.team2_players):
            player_id = participant.player_id
