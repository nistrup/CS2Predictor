"""Unit tests for player-level OpenSkill calculations."""
...
def test_map_win_updates_player_mu() -> None:
    calculator = PlayerOpenSkillCalculator(OpenSkillParameters())
    ...
    assert all(event.post_mu > event.pre_mu for event in winners)
    assert all(event.post_mu < event.pre_mu for event in losers)


def test_match_win_updates_player_mu() -> None:
    calculator = PlayerMatchOpenSkillCalculator(OpenSkillParameters())
...
def test_map_specific_blend_weight_increases_after_history() -> None:
    calculator = PlayerMapSpecificOpenSkillCalculator(MapSpecificOpenSkillParameters(map_prior_games=20.0))
